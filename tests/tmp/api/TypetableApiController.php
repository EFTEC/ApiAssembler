...
class TypetableApiController
{
    ...
    public function listallAction($id = null, $idparent = null, $event = null): void
    {
        ...
        if ($auth===false) {
            $this->api->errorShow(403, 'not allowed', 'Typetable/listall');
            die(1);
        }
        $body = $this->api->routeOne->getBody(true);
        try {
            // you can edit this part
            $result = TypetableRepo::toList();
            // end of the edit
            } catch (Exception $e) {
            $this->api->errorShow(500, 'unable to listall Typetable: '
                . $e->getMessage(), 'Typetable/listall');
            die(1);
        }
