...
use \ProducttypeAutoRepo;
use eftec\ApiAssembler\ApiAssemblerRuntime;
use Exception;
use RuntimeException;

/**
 * class ProducttypeAutoApiController
 *
...
class ProducttypeAutoApiController
{
    ...
    public function listallAction($id = null, $idparent = null, $event = null): void
    {
        ...
        if ($auth===false) {
            $this->api->errorShow(403, 'not allowed', 'ProducttypeAuto/listall');
            die(1);
        }
        $body = $this->api->routeOne->getBody(true);
        try {
            // you can edit this part
            $result = ProducttypeAutoRepo::toList();
            // end of the edit
            } catch (Exception $e) {
            $this->api->errorShow(500, 'unable to listall ProducttypeAuto: '
                . $e->getMessage(), 'ProducttypeAuto/listall');
            die(1);
        }
