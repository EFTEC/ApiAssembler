...
/**
 * class TablaparentApiController
 *
...
class TablaparentApiController
{
    ...
    public function listallAction($id = null, $idparent = null, $event = null): void
    {
        ...
        if ($auth===false) {
            $this->api->errorShow(403, 'not allowed', 'Tablaparent/listall');
            die(1);
        }
        $body = $this->api->routeOne->getBody(true);
        try {
            // you can edit this part
            $result = TablaparentRepo::toList();
            // end of the edit
            } catch (Exception $e) {
            $this->api->errorShow(500, 'unable to listall Tablaparent: '
                . $e->getMessage(), 'Tablaparent/listall');
            die(1);
        }
