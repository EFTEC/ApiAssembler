...
use \TablagrandchildRepo;
use eftec\ApiAssembler\ApiAssemblerRuntime;
use Exception;
use RuntimeException;

/**
 * class TablagrandchildApiController
 *
...
class TablagrandchildApiController
{
    ...
    public function listallAction($id = null, $idparent = null, $event = null): void
    {
        ...
        if ($auth===false) {
            $this->api->errorShow(403, 'not allowed', 'Tablagrandchild/listall');
            die(1);
        }
        $body = $this->api->routeOne->getBody(true);
        try {
            // you can edit this part
            $result = TablagrandchildRepo::toList();
            // end of the edit
            } catch (Exception $e) {
            $this->api->errorShow(500, 'unable to listall Tablagrandchild: '
                . $e->getMessage(), 'Tablagrandchild/listall');
            die(1);
        }
