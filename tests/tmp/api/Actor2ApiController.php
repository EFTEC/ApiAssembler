...
class Actor2ApiController
{
    ...
    public function listallAction($id = null, $idparent = null, $event = null): void
    {
        ...
        if ($auth===false) {
            $this->api->errorShow(403, 'not allowed', 'Actor2/listall');
            die(1);
        }
        $body = $this->api->routeOne->getBody(true);
        try {
            // you can edit this part
            $result = Actor2Repo::toList();
            // end of the edit
            } catch (Exception $e) {
            $this->api->errorShow(500, 'unable to listall Actor2: '
                . $e->getMessage(), 'Actor2/listall');
            die(1);
        }
