...
/**
* Class AbstractPaymentRepo. Copyright (c) Jorge Castro C. (https://github.com/EFTEC/PdoOne)<br>
...
abstract class AbstractPaymentRepo extends BaseSakila
{
    ...
    public static function convertOutputVal(&$row)
    {
        if ($row === false || $row === null) {
            return;
        		...
        		!isset($row['payment_id']) and $row['payment_id']=null; // smallint
		...
		!isset($row['amount']) and $row['amount']=null; // decimal
		!isset($row['payment_date']) and $row['payment_date']=null; // datetime
		!isset($row['last_update']) and $row['last_update']=null; // timestamp
        		isset($row['_customer_id'])
            and $row['_customer_id']['customer_id']=&$row['customer_id']; // linked MANYTOONE
		isset($row['_rental_id'])
            and $row['_rental_id']['rental_id']=&$row['rental_id']; // linked MANYTOONE
		isset($row['_staff_id'])
            and $row['_staff_id']['staff_id']=&$row['staff_id']; // linked MANYTOONE
    ...
    */
    ...
    {
        return static::newQuery()->where($sql, $param,false,PaymentRepo::TABLE);
    }

    public static function getDefFK($structure=false) {
        if ($structure) {
            return [
			    'customer_id' => 'FOREIGN KEY REFERENCES`customer`(`customer_id`) ON UPDATE CASCADE ON DELETE CASCADE',
			    'rental_id' => 'FOREIGN KEY REFERENCES`rental`(`rental_id`) ON UPDATE CASCADE ON DELETE CASCADE',
			    'staff_id' => 'FOREIGN KEY REFERENCES`staff`(`staff_id`) ON UPDATE CASCADE ON DELETE CASCADE'
			];
        }
    ...
    */
    public static function setRecursive($recursive=[])
    {
        if(is_string($recursive)) {
            $recursive=PaymentRepo::getRelations($recursive);
        }
    ...
    */
    public static function factory($values = null, $recursivePrefix = '') {
		...
		'payment_id'=>0,
		'customer_id'=>0,
		'_customer_id'=>(in_array($recursivePrefix.'_customer_id',$recursive,true)) 
		                            ? CustomerRepo::factory(null,$recursivePrefix.'_customer_id') 
		                            : null, /* MANYTOONE!! */
		'staff_id'=>0,
		'_staff_id'=>(in_array($recursivePrefix.'_staff_id',$recursive,true)) 
		                            ? StaffRepo::factory(null,$recursivePrefix.'_staff_id') 
		                            : null, /* MANYTOONE!! */
		'rental_id'=>0,
		'_rental_id'=>(in_array($recursivePrefix.'_rental_id',$recursive,true)) 
		                            ? RentalRepo::factory(null,$recursivePrefix.'_rental_id') 
		                            : null, /* MANYTOONE!! */
		'amount'=>0.0,
		'payment_date'=>'',
		'last_update'=>''
    ...
    */
    public static function factoryNull($values=null) {
		...
		'payment_id'=>null,
		'customer_id'=>null,
		'_customer_id'=>null, /* MANYTOONE!! */
		'staff_id'=>null,
		'_staff_id'=>null, /* MANYTOONE!! */
		'rental_id'=>null,
		'_rental_id'=>null, /* MANYTOONE!! */
		'amount'=>null,
		'payment_date'=>null,
		'last_update'=>null
		];
