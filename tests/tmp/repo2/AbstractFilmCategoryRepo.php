...
/**
* Class AbstractFilmCategoryRepo. Copyright (c) Jorge Castro C. (https://github.com/EFTEC/PdoOne)<br>
...
abstract class AbstractFilmCategoryRepo extends BaseSakila
{
    ...
    */
    public static function convertOutputVal(&$row)
    {
		...
		!isset($row['category_id']) and $row['category_id']=null; // tinyint
		!isset($row['last_update']) and $row['last_update']=null; // timestamp
        		isset($row['_category_id'])
            and $row['_category_id']['category_id']=&$row['category_id']; // linked MANYTOONE
		isset($row['_film_id'])
            and $row['_film_id']['film_id']=&$row['film_id']; // linked ONETOONE
    ...
    */
    ...
    {
        return static::newQuery()->where($sql, $param,false,FilmCategoryRepo::TABLE);
    }

    public static function getDefFK($structure=false) {
        if ($structure) {
            return [
			    'category_id' => 'FOREIGN KEY REFERENCES`category`(`category_id`) ON UPDATE CASCADE',
			    'film_id' => 'FOREIGN KEY REFERENCES`film`(`film_id`) ON UPDATE CASCADE'
			];
        }
    ...
    */
    public static function setRecursive($recursive=[])
    {
        if(is_string($recursive)) {
            $recursive=FilmCategoryRepo::getRelations($recursive);
        }
    ...
    */
    public static function factory($values = null, $recursivePrefix = '') {
        ...
        $row= [
		'film_id'=>0,
		'_film_id'=>(in_array($recursivePrefix.'_film_id',$recursive,true)) 
		                            ? FilmRepo::factory(null,$recursivePrefix.'_film_id') 
		                            : null, /* ONETOONE!! */
		'category_id'=>0,
		'_category_id'=>(in_array($recursivePrefix.'_category_id',$recursive,true)) 
		                            ? CategoryRepo::factory(null,$recursivePrefix.'_category_id') 
		                            : null, /* MANYTOONE!! */
		'last_update'=>''
		];
        		isset($row['_category_id'])
    ...
    */
    public static function factoryNull($values=null) {
        $row= [
		'film_id'=>null,
		'_film_id'=>null, /* ONETOONE!! */
		'category_id'=>null,
		'_category_id'=>null, /* MANYTOONE!! */
		'last_update'=>null
		];
        if ($values !== null) {
            $row = array_merge($row, $values);
