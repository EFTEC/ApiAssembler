...
/**
* Class AbstractTabletestRepo. Copyright (c) Jorge Castro C. (https://github.com/EFTEC/PdoOne)<br>
...
abstract class AbstractTabletestRepo extends BaseSakila
{
    ...
    public static function convertOutputVal(&$row)
    {
        if ($row === false || $row === null) {
            return;
        		...
        		!isset($row['id']) and $row['id']=null; // int
		!isset($row['col1']) and $row['col1']=null; // decimal
		!isset($row['col2']) and $row['col2']=null; // varchar
		!isset($row['col3']) and $row['col3']=null; // varchar
		!isset($row['col4']) and $row['col4']=null; // int
        		isset($row['_col4'])
            and $row['_col4']['staff_id']=&$row['col4']; // linked MANYTOONE
    ...
    */
    ...
    {
        return static::newQuery()->where($sql, $param,false,TabletestRepo::TABLE);
    }

    public static function getDefFK($structure=false) {
        if ($structure) {
            return [
			    'col4' => 'FOREIGN KEY REFERENCES`staff`(`staff_id`) ON UPDATE CASCADE ON DELETE CASCADE'
			];
        }
    ...
    */
    public static function setRecursive($recursive=[])
    {
        if(is_string($recursive)) {
            $recursive=TabletestRepo::getRelations($recursive);
        }
    ...
    */
    public static function factory($values = null, $recursivePrefix = '') {
		...
		'col3'=>'',
		'col4'=>0,
		'_col4'=>(in_array($recursivePrefix.'_col4',$recursive,true)) 
		                            ? StaffRepo::factory(null,$recursivePrefix.'_col4') 
		                            : null, /* MANYTOONE!! */
		];
        		isset($row['_col4'])
            and $row['_col4']['staff_id']=&$row['col4']; // linked MANYTOONE
