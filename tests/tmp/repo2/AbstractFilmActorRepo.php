...
/**
* Class AbstractFilmActorRepo. Copyright (c) Jorge Castro C. (https://github.com/EFTEC/PdoOne)<br>
...
abstract class AbstractFilmActorRepo extends BaseSakila
{
    ...
    public static function convertOutputVal(&$row)
    {
        if ($row === false || $row === null) {
            return;
        		...
        		!isset($row['actor_id']) and $row['actor_id']=null; // smallint
		!isset($row['film_id']) and $row['film_id']=null; // smallint
		!isset($row['last_update']) and $row['last_update']=null; // timestamp
        		isset($row['_actor_id'])
            and $row['_actor_id']['actor_id']=&$row['actor_id']; // linked ONETOONE
		isset($row['_film_id'])
            and $row['_film_id']['film_id']=&$row['film_id']; // linked MANYTOONE
    ...
    */
    ...
    {
        return static::newQuery()->where($sql, $param,false,FilmActorRepo::TABLE);
    }

    public static function getDefFK($structure=false) {
        if ($structure) {
            return [
			    'actor_id' => 'FOREIGN KEY REFERENCES`actor`(`actor_id`) ON UPDATE CASCADE',
    ...
    */
    public static function setRecursive($recursive=[])
    {
        if(is_string($recursive)) {
            $recursive=FilmActorRepo::getRelations($recursive);
        }
    ...
    */
    public static function factory($values = null, $recursivePrefix = '') {
        ...
        $row= [
		'actor_id'=>0,
		'_actor_id'=>(in_array($recursivePrefix.'_actor_id',$recursive,true)) 
		                            ? ActorRepo::factory(null,$recursivePrefix.'_actor_id') 
		                            : null, /* ONETOONE!! */
		'film_id'=>0,
    ...
    */
    public static function factoryNull($values=null) {
        $row= [
		'actor_id'=>null,
		'_actor_id'=>null, /* ONETOONE!! */
		'film_id'=>null,
		'_film_id'=>null, /* MANYTOONE!! */
		'last_update'=>null
		];
        if ($values !== null) {
            $row = array_merge($row, $values);
