...
/**
* Class AbstractTablachildRepo. Copyright (c) Jorge Castro C. (https://github.com/EFTEC/PdoOne)<br>
...
* <pre>
* $code=$pdoOne->generateCodeClass('tablachild','eftec\tests\tmp\repo2',array(),array('actor'=>'ActorRepo','actor2'=>'Actor2Repo','address'=>'AddresRepo','category'=>'CategoryRepo','city'=>'CityRepo','country'=>'CountryRepo','customer'=>'CustomerRepo','dummyt'=>'DummytRepo','dummytable'=>'DummytableRepo','film'=>'FilmRepo','film2'=>'Film2Repo','film_actor'=>'FilmActorRepo','film_category'=>'FilmCategoryRepo','film_text'=>'FilmTextRepo','fum_jobs'=>'FumJobRepo','fum_logs'=>'FumLogRepo','inventory'=>'InventoryRepo','language'=>'LanguageRepo','mysec_table'=>'MysecTableRepo','payment'=>'PaymentRepo','product'=>'ProductRepo','producttype'=>'ProducttypeRepo','producttype_auto'=>'ProducttypeAutoRepo','rental'=>'RentalRepo','staff'=>'StaffRepo','store'=>'StoreRepo','tablachild'=>'TablachildRepo','tablagrandchild'=>'TablagrandchildRepo','tablaparent'=>'TablaparentRepo','tabletest'=>'TabletestRepo','test_products'=>'TestProductRepo','typetable'=>'TypetableRepo',),array(),'','','BaseSakila','',array(),array());
* </pre>
*/
abstract class AbstractTablachildRepo extends BaseSakila
{
    public const TABLE = 'tablachild';
    public const IDENTITY = 'idtablachild';
    public const PK = [
	    'idtablachild'
	];
    ...
    public static function convertOutputVal(&$row)
    {
        if ($row === false || $row === null) {
            return;
        		...
        		!isset($row['idtablachild']) and $row['idtablachild']=null; // int
		!isset($row['valuechild']) and $row['valuechild']=null; // varbinary
		!isset($row['idgrandchildFK']) and $row['idgrandchildFK']=null; // int
        		isset($row['_idgrandchildFK'])
            and $row['_idgrandchildFK']['idgrandchild']=&$row['idgrandchildFK']; // linked MANYTOONE
    ...
    */
    ...
    {
        return static::newQuery()->where($sql, $param,false,TablachildRepo::TABLE);
    }
    ...
    */
    public static function setRecursive($recursive=[])
    {
        if(is_string($recursive)) {
            $recursive=TablachildRepo::getRelations($recursive);
        }
    ...
    */
    public static function factory($values = null, $recursivePrefix = '') {
        ...
        $row= [
		'idtablachild'=>0,
		'_tablaparent'=>(in_array($recursivePrefix.'_tablaparent',$recursive,true))
		                            ? [] 
		                            : null, /* ONETOMANY! */
		'valuechild'=>'',
		'idgrandchildFK'=>0,
		'_idgrandchildFK'=>(in_array($recursivePrefix.'_idgrandchildFK',$recursive,true)) 
		                            ? TablagrandchildRepo::factory(null,$recursivePrefix.'_idgrandchildFK') 
		                            : null, /* MANYTOONE!! */
		];
        		isset($row['_idgrandchildFK'])
            and $row['_idgrandchildFK']['idgrandchild']=&$row['idgrandchildFK']; // linked MANYTOONE
